...
class ProductManager:
    ...
    def get_product(self, product_id: str) -> ProductAd:
        ...
        """
        response = requests.get(f'{self.url}/{product_id}', headers={'Authorization': f'Bearer {self.access_token}'})
    ...
    def create_product(self, product: ProductCreate) -> ProductAd:
        ...
        """
        response = requests.post(self.url, headers={'Authorization': f'Bearer {self.access_token}'}, json=product.dict())
        response.raise_for_status()
    ...
    def update_product(self, product_id: str, product: ProductUpdate) -> ProductAd:
        ...
        """
        response = requests.put(f'{self.url}/{product_id}', headers={'Authorization': f'Bearer {self.access_token}'}, json=product.dict())
        response.raise_for_status()
        return ProductAd(**response.json())
    ...
    def close_product(self, product_id: str) -> ProductAd:
        ...
        """
        response = requests.put(f'{self.url}/{product_id}', headers={'Authorization': f'Bearer {self.access_token}'}, json={'status': 'closed'})
        response.raise_for_status()
        return ProductAd(**response.json())
    ...
    def delete_product(self, product_id: str) -> ProductAd:
        ...
        response = requests.put(f'{self.url}/{product_id}', headers={'Authorization': f'Bearer {self.access_token}'}, json={'deleted': True})
        response.raise_for_status()
        return ProductAd(**response.json())
    ...
    def get_product_description(self, product_id: str) -> ProductDescription:
        ...
        """
        response = requests.get(f'{self.url}/{product_id}/description', headers={'Authorization': f'Bearer {self.access_token}'})
        response.raise_for_status()
        return ProductDescription(**response.json())
    
    def update_product_description(self, product_id: str, description: ProductDescription) -> ProductDescription:
        """
        Atualiza a descrição de um produto no Mercado Livre.
        :param product_id: ID do produto.
        :param description: Descrição do produto.
        """
        response = requests.put(f'{self.url}/{product_id}/description', headers={'Authorization': f'Bearer {self.access_token}'}, json=description.dict())
        response.raise_for_status()
        return ProductDescription(**response.json())